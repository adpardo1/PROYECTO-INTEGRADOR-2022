...
SELECT  p.nombre_provincia "PROVINCIA", count(e.nombre_esta) "TOTAL ESTABLECIMINETOS", SUM(e.estimacion) "FLUJO DE CLIENTES NAYOR AL PROMEDIO"
...
-- -----------------------------------------------------------------------------------------------------------------------
-- establecimiento de 5 estrellas 
SELECT p.nombre_provincia"PROVINCIA", count(t.nombre_esta)"TOTAL DE ESTABLECIMIENTOS", c.categoria "CATEGORIA"
FROM esta_provin e INNER JOIN establecimiento t ON e.establecimiento_idestablecimiento = t.idestablecimiento
				   INNER JOIN provincia p ON e.Provincia_idProvincia = p.idProvincia
                   INNER JOIN categoria c ON t.idestablecimiento = c.establecimiento_idestablecimiento
WHERE c.categoria like '5%'
GROUP BY 1
ORDER BY 2 DESC
Limit 5;
